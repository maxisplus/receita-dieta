...
import { weekMenu, Meal, MealOption } from '@/data/menu';
import { getWaterIntakeFromLocalStorage } from '@/lib/waterCalculator';
import { getUser, isAuthenticated, clearUser } from '@/lib/auth';
...
export default function Home() {
  ...
  const navigate = (dir: 'prev' | 'next') => {
    ...
    setDay((d) => (dir === 'prev' ? Math.max(1, d - 1) : Math.min(maxDay, d + 1)));
  };
      ...
      <div className="max-w-[620px] mx-auto">
        ...
        <header className="mb-4 sm:mb-6">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex-1 flex flex-col items-center text-center min-w-0">
              <p className="text-xs sm:text-sm md:text-base font-medium bg-gradient-to-r from-[rgb(255,145,26)] via-[rgb(255,165,0)] to-[rgb(255,145,26)] bg-clip-text text-transparent animate-gradient-shimmer">
                Desafio dos 21 dias
              </p>
              <p className="text-sm sm:text-base md:text-lg lg:text-xl font-medium text-gray-600">
                Olá, <span className="font-semibold bg-gradient-to-r from-[rgb(255,145,26)] to-[rgb(255,165,0)] bg-clip-text text-transparent">{user.name}</span>
              </p>
            </div>
        ...
        <div className="flex items-center gap-2 mb-4 sm:mb-6 md:mb-8">
          ...
          >
            ...
            <span className="truncate">{water ? `${water}L água` : 'Calcular água'}</span>
          </button>

          <button className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3 md:px-4 py-2 sm:py-2.5 bg-gradient-to-r from-[rgb(255,145,26)] to-[rgb(255,165,0)] text-white rounded-lg sm:rounded-xl text-[11px] sm:text-xs md:text-sm font-semibold
            ...
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 relative z-10" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        ...
        {/* ─── CARD PRINCIPAL ─── */}
        <div className="bg-white rounded-2xl border-2 border-transparent bg-clip-padding overflow-hidden relative
              ...
              <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
                ...
                <div className="text-center">
                  <h2 className="text-xl font-bold">
                    ...
                    <span className="text-sm font-normal text-gray-400 ml-1.5">de {maxDay}</span>
                  </h2>
        ...
        <p className="text-center text-xs text-gray-400 mt-8">
          Desafio dos 21 dias &middot; Cardápio fit personalizado
        </p>
      </div>

      {/* Modais */}
      <WaterCalculatorModal isOpen={showWater} onClose={() => setShowWater(false)} onCalculated={setWater} />
      ...
      >
        <div className="flex items-center gap-2">
          <div className="option-badge mb-0">
            <span className={`w-4 h-4 rounded-md inline-flex items-center justify-center text-[10px] font-bold transition-all ${
              isOpen 
            ...
            <span className={isOpen ? 'text-[rgb(255,145,26)] font-semibold' : ''}>Opção {index}</span>
          </div>
        </div>
        <svg
          className={`w-4 h-4 transition-all duration-200 ${
            isOpen 
              ? 'rotate-180 text-[rgb(255,145,26)]' 
              : 'text-gray-400'
          }`}
          fill="none"
          stroke="currentColor"
